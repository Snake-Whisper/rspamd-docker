...
RUN wget -qO- https://github.com/rspamd/rspamd/archive/refs/tags/${RSPAMD_VERSION}.tar.gz | tar -xz && \
    mkdir /rspamd.build && \
    cd /rspamd.build && \
    apk add --no-cache --virtual build-deps openssl-dev make cmake gcc g++ perl glib-dev sqlite-dev harfbuzz-dev libsodium-dev ragel lua-dev hyperscan-dev pcre2-dev && \
